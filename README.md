...
- **VS Code**: Expects the MesloLGS NF font (for terminal) and Fira Code (editor). The Vim extension (see `vscode/settings.json`) is configured for leader-based shortcuts, and `vscode/keybindings.json` adds R-friendly snippets like pipe insertion. Activity bar moves to the top and the sidebar to the right to maximize vertical space.
